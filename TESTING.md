...
Tests are organized in the `netbox_config/tests/` directory:
...
```python
from netbox_config.testing import PluginTestCase
...
```python
from netbox_config.testing import PluginModelTestCase
...
```python
from netbox_config.testing import PluginAPITestCase

class MyAPITestCase(PluginAPITestCase):
    def setUp(self):
        super().setUp()
        self.list_url_name = 'plugins-api:netbox_config-api:mymodel-list'
        self.detail_url_name = 'plugins-api:netbox_config-api:mymodel-detail'
...
```python
from netbox_config.testing import PluginViewTestCase

class MyViewTestCase(PluginViewTestCase):
    def setUp(self):
        super().setUp()
        self.base_url = 'plugins:netbox_config:mymodel'
...
```python
from netbox_config.testing import PluginGraphQLTestCase
...
The `netbox_config.testing.utils` module provides helpful utilities:

```python
from netbox_config.testing.utils import (
...
user = create_test_user(permissions=['netbox_config.view_config'])
...
python manage.py test netbox_config.tests.test_models

# Run specific test class
python manage.py test netbox_config.tests.test_models.ConfigTestCase

# Run specific test method
python manage.py test netbox_config.tests.test_models.ConfigTestCase.test_create

# Run with verbose output
python manage.py test netbox_config.tests -v 2

# Run in parallel (faster)
python manage.py test netbox_config.tests --parallel

# Keep database between runs (faster during development)
python manage.py test netbox_config.tests --keepdb
```
...
# Run tests
docker-compose exec netbox python manage.py test netbox_config.tests

# Run specific tests
docker-compose exec netbox python manage.py test netbox_config.tests.test_api
...
def test_form_validation(self):
    ...
    url = reverse('netbox_config:mymodel_add')
