"""
Test cases for NetBox Config Plugin models.
"""

from django.core.exceptions import ValidationError

from ..models import Config
from ..testing import PluginModelTestCase
from ..testing.utils import create_tags, get_random_string


class ConfigTestCase(PluginModelTestCase):
    """Test Config model."""

    @classmethod
    def setUpTestData(cls):
        """Set up test data for all tests."""
        # Create test instances
        Config.objects.create(name='Test 1')
        Config.objects.create(name='Test 2')
        Config.objects.create(name='Test 3')

    def test_create_config(self):
        """Test creating a Config instance."""
        name = f'Test {get_random_string(10)}'
        instance = Config.objects.create(name=name)

        self.assertEqual(instance.name, name)
        self.assertIsNotNone(instance.pk)

    def test_config_str(self):
        """Test Config string representation."""
        instance = Config.objects.first()
        self.assertEqual(str(instance), instance.name)

    def test_config_absolute_url(self):
        """Test Config get_absolute_url method."""
        instance = Config.objects.first()
        url = instance.get_absolute_url()

        self.assertIsNotNone(url)
        self.assertIn(str(instance.pk), url)

    def test_config_unique_name(self):
        """Test that Config names must be unique."""
        name = 'Duplicate Name'
        Config.objects.create(name=name)

        with self.assertRaises(ValidationError):
            instance = Config(name=name)
            instance.full_clean()

    def test_model_to_dict(self):
        """Test model_to_dict helper method."""
        instance = Config.objects.first()
        data = self.model_to_dict(instance)

        self.assertIn('name', data)
        self.assertEqual(data['name'], instance.name)
        self.assertIn('id', data)

    def test_instance_equal(self):
        """Test assertInstanceEqual helper method."""
        instance = Config.objects.first()

        # Should pass with matching data
        self.assertInstanceEqual(
            instance,
            {'name': instance.name, 'id': instance.pk}
        )

    def test_config_with_tags(self):
        """Test Config with tags."""
        tags = create_tags(['important', 'test'])
        instance = Config.objects.first()

        instance.tags.add(*tags)
        instance.save()

        self.assertEqual(instance.tags.count(), 2)
        self.assertIn(tags[0], instance.tags.all())

    def test_bulk_create(self):
        """Test bulk creation of Config instances."""
        initial_count = Config.objects.count()

        instances = [
            Config(name=f'Bulk {i}')
            for i in range(5)
        ]
        Config.objects.bulk_create(instances)

        self.assertEqual(
            Config.objects.count(),
            initial_count + 5
        )

    def test_query_filter(self):
        """Test filtering Config instances."""
        # Create a specific instance for filtering
        test_name = f'FilterTest {get_random_string(10)}'
        Config.objects.create(name=test_name)

        # Test filter
        results = Config.objects.filter(name=test_name)
        self.assertEqual(results.count(), 1)
        self.assertEqual(results.first().name, test_name)

    def test_ordering(self):
        """Test Config default ordering."""
        instances = list(Config.objects.all())

        # Check that instances are ordered by name
        names = [instance.name for instance in instances]
        self.assertEqual(names, sorted(names))


class ConfigValidationTestCase(PluginModelTestCase):
    """Test Config validation."""

    def test_empty_name(self):
        """Test that empty name is not allowed."""
        with self.assertRaises(ValidationError):
            instance = Config(name='')
            instance.full_clean()

    def test_name_max_length(self):
        """Test name field max length."""
        long_name = 'x' * 101  # Exceeds max_length of 100

        with self.assertRaises(ValidationError):
            instance = Config(name=long_name)
            instance.full_clean()
