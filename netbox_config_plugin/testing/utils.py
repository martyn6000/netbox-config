"""
Utility functions for testing NetBox Config Plugin.
...
def create_test_user(username: str | None = None, permissions: list[str | None] = None,
    ...
    Example:
        ...
        ...     permissions=['netbox_config_plugin.view_config']
        ... )
    """
    if username is None:
...
def get_deletable_objects(instances):
    ...
    Example:
        >>> objs = Config.objects.filter(name__startswith='test')
        >>> deletions = get_deletable_objects(objs)
        >>> # deletions = {'Config': 3, 'RelatedModel': 5}
    """
...
def assert_object_changes(test_case, instance, action: str,
    ...
    Example:
        ...
        >>> obj = Config.objects.create(name='Test')
